...
class P5JsUtils {
    ...
    static colorAt(
        ...
        return P5JsUtils.colorAtInPixels(p5, x, y, width, p5.pixels);
    }

    static colorAtInPixels(
        p5: p5InstanceExtensions,
        x: number,
        y: number,
        width: number,
        ...
        let baseIdx = (p5.round(x) + p5.round(y) * width) * 4;
        return p5.color(
            pixels[baseIdx + 0],
            pixels[baseIdx + 1],
            pixels[baseIdx + 2],
    ...
    static mousePoint(p5: p5InstanceExtensions) {
    ...
    static get COLOR_ID_RED() {
        return 0;
    }
    static get COLOR_ID_GREEN() {
        return 1;
    }
    static get COLOR_ID_BLUE() {
        return 2;
    }
    static get COLOR_ID_ORANGE() {
        return 3;
    }
    static get COLOR_ID_YELLOW() {
        return 4;
    }
    static get COLOR_ID_VIOLET() {
        return 5;
    }

    //   static getRandomColorByID(colorId: number){
    //     switch(colorId) {
    ...
    static applyStyleSet(
        p5: p5InstanceExtensions,
        styleSet: any,
        buffer?: p5InstanceExtensions
    ) {
        if (buffer) {
            ...
            if (styleSet.fillColor) {
                p5.fill(styleSet.fillColor);
            }
            if (styleSet.noFill) {
                p5.noFill();
            }
            if (styleSet.strokeColor) {
                p5.stroke(styleSet.strokeColor);
            }
            if (styleSet.noStroke) {
                p5.noStroke();
            }
            if (styleSet.strokeWeight) {
                p5.strokeWeight(styleSet.strokeWeight);
            }
        }
    }

    static drawControlPoints(p5: p5InstanceExtensions, points: Point[]) {
        p5.push();
        points.forEach((p) => P5JsUtils._drawControlPoint(p5, p));
        p5.pop();
    }

    static _drawControlPoint(p5: p5InstanceExtensions, point: Point) {
        if (point.containsXY(p5, p5.mouseX, p5.mouseY)) {
            p5.fill(200, 200, 100);
        } else {
            p5.fill(100, 200, 100);
        }
        p5.noStroke();
        p5.ellipse(point.x, point.y, point.radius, point.radius);
    }
}
