...
const Edit: NextPage<BaseProps> = ({}) => {
    ...
    return (
        <div className={' w-full h-full flex overflow-y-scroll'}>
            <div id={'imgscroll'} className={'min-w-[230px] min-h-[920px] w-[320px] h-full p-2 bg-slate-100 overflow-scroll '}>
                {imglist.map((val,key,)=>{
                    return(
                        <div key={key} className={'relative self-center w-full'}>
                            <img className={'relative w-full h-[200px] my-2 rounded-xl'} src={val.src}
                            onClick={()=>{
                                setindex(key);
                                settitle(val.title);
                            ...
                            <img src="https://img.icons8.com/material-rounded/24/000000/trash.png"
                                className={'absolute top-[0px] right-[0px] w-auto h-auto cursor-pointer'} 
                                onClick={(e)=>{  
                ...
                <div className={'flex p-5 flex-col space-y-5 left-0 top-0 w-[96%] h-[96%] bg-slate-200 m-[2%] rounded-3xl'}>
                    <div className={'self-center'}>
                        <div className={' w-[600px] h-[400px] bg-zinc-400'}>  
                            <Canvas2 img={[img]} update_w_h={update_widthheight} value={[title,descript]}/>
                        </div>
                    </div>
                    <div className={'flex flex-row w-[100%] h-[55%]  space-x-1 '}>
                        <div className={'flex flex-col w-full h-[100%] rounded-3xl border-gray-400 border-solid border-[1px] p-3'}>
                            ...
                            <div {...getRootProps()} className={'flex flex-col w-full h-full bg-white rounded-md cursor-pointer'}>
                                <input {...getInputProps()} id={'imginput'} type={'file'} onChange={imgload} />{
                                    imgcheck?
                                    <div className={'w-auto h-auto text-center leading-[300px]'}><img  src={img}/></div>:
                                    isDragActive ?
                                    <div className={'text-center leading-[300px]'}><PulseLoader  color="#8BCB6A" size={20} margin="1px" /></div> :
                                    <div className={'text-center w-full h-full leading-[300px]'}><p>이곳에 이미지를 끌어오세요</p></div>
                                }
                            </div>   
                        </div>
                        <div className={'flex flex-col w-full space-y-1'}>
                            ...
                            <div className={'flex flex-row  w-full h-[25%] rounded-3xl border-gray-400 border-solid border-[1px] p-3 space-x-1'}>
                                <div className={'flex flex-col w-[100%]'}>
                                    ...
                                    <input className={'w-auto'} disabled={true} type={'number'} onChange={update_width} value={imgwidth} ></input>
                                </div>
                                <div className={'flex flex-col w-[100%]'}>
                                    <div className={'text-[20px] font-bold w-full border-b-[1px] m-1 border-solid border-gray-400'}>세로 길이</div>
                                    <input className={'w-auto'} disabled={true} type={'number'} onChange={update_height} value={imgheight} ></input>
                                </div>
